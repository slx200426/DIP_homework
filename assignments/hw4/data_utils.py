...
def read_points3D_text(path):
    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            error = float(data[7])
            points3D[point_id] = {'xyz': xyz, 'rgb': rgb, 'error': error}
    return points3D
...
def sample_farthest_points(points, K):
    ...
    to replace pytorch3d.ops.sample_farthest_points on Mac.
    Args:
        points: (B, N, 3) tensor of points
        K: Number of points to sample
    Returns:
        (sampled_points, indices)
    """
    ...
    sampled_pts = points[batch_indices.unsqueeze(-1), centroids]
    return sampled_pts, centroids
...
class ColmapDataset(Dataset):
    ...
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num=3000):
        ...
        # Load COLMAP data
        self.cameras = read_cameras_text(
            os.path.join(sparse_path, "cameras.txt"))
        self.images = read_images_text(os.path.join(sparse_path, "images.txt"))
        points3D = read_points3D_text(os.path.join(sparse_path,
                                                   "points3D.txt"))

        # Convert points3D to torch.tensor
        self.points3D_xyz = torch.as_tensor(
            np.array([p['xyz'] for p in points3D.values()])).float()
        self.points3D_rgb = torch.as_tensor(
            np.array([p['rgb'] for p in points3D.values()])).float()
